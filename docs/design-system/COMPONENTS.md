...
| **Chip and tab edge fade** | `.nv-chips` (all widths), `.nv-tabs` below 1000 px | A mask fade signals horizontal scroll without a scrollbar. The fade sits in the chip row's right padding, so a row that fits is unaffected. |
| **List tiers by container width** | Every `.nv-rows` list | The list is a size container (`container: rows / inline-size`) and lays out by the room it actually has, not the viewport: **card** under 600 px; **tablet row** at 600–1039 px (title, status and summary left, actions right); **full column table** at 1040 px or more. The tiers matter because the rail and sidebar take room the viewport width doesn't show. Browsers without container queries keep the card and a viewport-based table. |
| **Product detail** (`.nv-pd`) | Inventory drawer | Identity line (category · brand · unit) and badges → the **on-hand** block on the surface its status earns (critical, decision or raised) with the stock figure, meter, reorder scale, days of stock, suggested reorder and the two actions → the risk alert → a statement for expiry and for cost and value → stock history (`.nv-moves`: direction icon, note, relative time, signed tabular delta). It shows only recorded fields. |
| **Team** (`.nv-member`, `.nv-invites`, `.nv-role-pick`) | Staff | A pulse strip → member cards (initials avatar, status and role badges, last activity, 7-day sales, actions) → invitations → the audit log as an activity timeline. Owner-only actions sit in a fixed column on desktop and under a divider on phones. The invite role is a real radio group. |
| **Document rows** (`.nv-doc`, `.nv-dropzone`) | Documents | A file-type tile (PDF, image, spreadsheet or document, from the name), then the name and meta (category icon, size, date), tags and note, then an expiry badge (Expired / Expires in N days / Valid to) and View / Download icon buttons with full names. The upload surface is a real button that opens the file picker. Categories use Lucide icons, never emoji. |
