...
import { Badge, Button, Card, EmptyState, MetricCard, PageHeader, SkeletonBlock } from "@/platform/ui";
import { CalendarClock, CircleCheck } from "@/platform/ui/icons";

const GROUPS = [
  { id: "expired", title: "Expired", tone: "danger", hint: "Remove from sale now." },
  { id: "urgent", title: "Urgent · within 7 days", tone: "danger", hint: "Dispense first, today." },
  { id: "d30", title: "Within 30 days", tone: "warning", hint: "Dispense first; don’t reorder yet." },
  { id: "d60", title: "30–60 days", tone: "info", hint: "Watch; order smaller quantities." },
  { id: "d90", title: "60–90 days", tone: "neutral", hint: "On the radar." }
];
  ...
  return (
    <div className="nv-page">
      ...
      <div className="nv-summary">
        <MetricCard label="Already expired" value={fmt(expiredValue)} sub={`${byGroup.expired.length} product${byGroup.expired.length === 1 ? "" : "s"} · at cost`} />
        <MetricCard label="Likely to expire unsold (≤30 days)" value={fmt(nearRisk)} sub="at cost, from recorded sales rates" />
        <MetricCard label="Expiring in 30–90 days" value={fmt(laterValue)} sub="stock value at cost" />
      </div>
        ...
        <div className="nv-stack">
          {GROUPS.filter((g) => byGroup[g.id].length > 0).map((g) => (
            <section key={g.id} aria-labelledby={`exp-${g.id}`}>
              ...
              <div className="nv-rows" role="list" aria-label={g.title} style={{ "--cols": "minmax(180px, 1.6fr) 110px 120px 120px minmax(200px, 2fr) auto" }}>
                <div className="nv-rows__head" aria-hidden="true"><span>Medicine</span><span>Quantity</span><span>Expires</span><span>Value at risk</span><span>Recommended</span><span /></div>
                {byGroup[g.id].map((p) => {
                  const rec = recommendation(p);
                  const risk = p.expiry === "expired" ? p.valueAtCost : p.valueAtRiskAtExpiry;
                  return (
                    <div key={p.id} role="listitem" className={`nv-row${g.tone === "danger" ? " nv-row--attention" : ""}`}>
                      ...
                      <div className="nv-row__meta">
                        <strong>{p.stock} {p.unit}</strong> · {p.expiry === "expired" ? "expired" : "expires"} {fmtDate(p.expiryDate)}{risk > 0 ? ` · ${fmt(risk)} at risk` : ""}
                        <div style={{ marginTop: 4 }}>{rec.text}</div>
                      </div>
                      <div className="nv-row__cell nv-num">{p.stock} {p.unit}</div>
                      <div className="nv-row__cell">{fmtDate(p.expiryDate)}</div>
                      <div className="nv-row__cell nv-num">{risk > 0 ? fmt(risk) : "—"}<small>{p.expiry === "expired" ? "all stock" : "unsold at expiry"}</small></div>
                      <div className="nv-row__cell" style={{ fontSize: "0.875rem" }}>{rec.text}</div>
      ...
      {noDate.length > 0 && total > 0 && (
        <p className="nv-hint" style={{ marginTop: 16, display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {onNavigate && <button type="button" className="nv-link" style={{ background: "none", border: 0, padding: 0, minHeight: 0, cursor: "pointer" }} onClick={() => onNavigate("inventory")}>Open Inventory</button>}
        </p>
      )}
